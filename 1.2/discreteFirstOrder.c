//gcc discreteFirstOrder.c -o disc -lglut -lGL -lm
#include <GL/glut.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
void place_point(double x, double y)
{
	glPointSize(5.0);
...
double function(double x)
{
	...
	//return(6.0 - 1.0*x*x);
	//return(6.0 - 0.1*x*x);
	//return(6.0 - 0.5*x*x); //start at 2 4 or -2 very unstable
	//return(6.0 - 0.25*x*x);
}
...
{	
	...
	if(state == GLUT_DOWN)
	{
		if(button == GLUT_LEFT_BUTTON)
		{
			...
			XPresent = 0.0;
			YPresent = y_machine_to_y_world(y);
			Count = 0;
			printf("\n\n  f[%d] = %f\n", Count, YPresent);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Discrete First Oder");
